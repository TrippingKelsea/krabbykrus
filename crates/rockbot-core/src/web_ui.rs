        ...
        <main class="main">
            ...
            <div id="page-models" class="content page hidden">
                ...
                <div class="grid" id="providers-grid">
                    <div class="card"><p class="text-dim">Loading providers from gateway...</p></div>
                </div>
            </div>
                ...
                <div class="grid">
                    ...
                    <div class="card">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" id="vault-path" readonly value="~/.config/rockbot/vault.db">
                        </div>
                    </div>
                </div>
                <div class="card mt-2">
                    <h3 class="mb-2">About</h3>
                    <p>RockBot - A Rust-native AI agent framework</p>
                    <p class="text-dim mt-1">https://github.com/openclaw/rockbot</p>
                </div>
            </div>
        </main>
    </div>
    ...
    <script>
        ...
        async function loadProviders() {
            ...
            try {
                ...
                if (providers.length === 0) {
                    grid.innerHTML = '<div class="card"><p class="text-dim">No providers registered. Is the gateway running?</p></div>';
                    return;
                }
                grid.innerHTML = providers.map(p => {
                    ...
                    const caps = ['streaming', 'tools', 'vision']
                        .filter(c => p['supports_' + c])
                        .map(c => `<span class="badge badge-info" style="margin-right:4px">${c}</span>`)
                        .join('');
                    const models = (p.models || []).slice(0, 5)
                        .map(m => `<li style="color:var(--text-dim);font-size:0.85em">${m.name}</li>`)
                        .join('');
                    const moreCount = (p.models || []).length - 5;
                    const moreText = moreCount > 0 ? `<li style="color:var(--text-dim);font-size:0.8em">...and ${moreCount} more</li>` : '';
                    return `<div class="card">
                        <div class="card-header"><h3>${p.name}</h3>${badge}</div>
                        <p class="text-dim mb-2">Auth: ${p.auth_type} &nbsp; ID: ${p.id}</p>
                        <div style="margin:0.5rem 0">${caps}</div>
                        ${models ? `<ul style="list-style:none;padding:0;margin:0.5rem 0">${models}${moreText}</ul>` : ''}
                        <button class="btn btn-primary btn-sm" onclick="testProvider('${p.id}')">Test Connection</button>
                    </div>`;
                }).join('');
            } catch (e) {
                grid.innerHTML = '<div class="card"><p style="color:var(--error)">Failed to load providers: ' + e.message + '</p></div>';
            }
        }
        ...
        async function testProvider(id) {
            try {
                ...
                if (res.status === 'ok') {
                    showSuccess(`${id}: connection OK (${res.models_found} models)`);
                } else {
                    showError(`${id}: ${res.error}`);
                }
            } catch (e) {
                showError(e.message);
            }
