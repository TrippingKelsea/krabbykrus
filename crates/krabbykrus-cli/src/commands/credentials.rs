...
async fn init_vault(
    ...
    if let Some(pubkey) = age_pubkey {
        ...
        println!("   Unlock with: krabbykrus credentials unlock --age <identity>");
        ...
        println!("   Unlock with: krabbykrus credentials unlock --ssh-key <private_key_path>");
    ...
    println!();
    println!("Next steps:");
    println!("  • Add credentials: krabbykrus credentials add <name> -t <type> -u <url>");
    println!("  • View status:     krabbykrus credentials status");
...
async fn add_credential(
    ...
    if !CredentialVault::exists(&config.credentials.vault_path) {
        anyhow::bail!("Vault not initialized. Run 'krabbykrus credentials init' first.");
    }
    ...
    if let Some(secret_value) = secret {
        ...
        println!("ℹ️  No secret provided. Use 'krabbykrus credentials add' with --secret to add later.");
    }
...
async fn view_audit(config: &Config, verify: bool, _limit: usize) -> Result<()> {
    ...
    if verify {
        ...
        println!("\nTo verify integrity: krabbykrus credentials audit --verify");
...
async fn unlock_vault(
    ...
    use krabbykrus_credentials::UnlockMethod;
