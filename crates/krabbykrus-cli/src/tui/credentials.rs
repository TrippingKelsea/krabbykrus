...
use krabbykrus_credentials::{CredentialVault, UnlockMethod};
        ...
        match &self.unlock_method {
            ...
            Some(UnlockMethod::Keyfile { path_hint }) => {
                ...
                let kf_path = path_hint.as_ref()
                    ...
                    .or_else(|| {
                        dirs::config_dir()
                            .map(|d| d.join("krabbykrus").join("vault.key"))
                    });
    ...
    fn render_settings(&self, frame: &mut Frame, area: Rect) {
        ...
        if !self.vault_exists {
            ...
            let text = vec![
                ...
                Line::from(Span::styled("Or use CLI: krabbykrus credentials init", Style::default().fg(Color::DarkGray))),
            ];
