//! Error handling for Krabbykrus core components

use std::path::PathBuf;
use thiserror::Error;

/// Main result type for Krabbykrus operations
pub type Result<T> = std::result::Result<T, KrabbykrusError>;

/// Hierarchical error system for Krabbykrus
#[derive(Debug, Error)]
pub enum KrabbykrusError {
    ...
    ExternalSecurity(#[from] krabbykrus_security::SecurityError),
    
    #[error("External tool error: {0}")]
    ExternalTool(#[from] krabbykrus_tools::ToolError),
    
    #[error("Credential error: {0}")]
    Credential(#[from] krabbykrus_credentials::CredentialError),
