//! Encrypted credential storage for krabbykrus-credentials.
//!
...
//! - `$XDG_DATA_HOME/krabbykrus/credentials/meta.json` - Vault metadata (salt, version)
//! - `$XDG_DATA_HOME/krabbykrus/credentials/endpoints.json` - Endpoint configurations
//! - `$XDG_DATA_HOME/krabbykrus/credentials/credentials.json` - Encrypted credential data
...
fn wrap_key_with_ssh(key_bytes: &[u8], public_key_path: &Path) -> Result<String> {
    ...
    hasher.update(b"krabbykrus-ssh-wrap-v1");
...
pub struct VaultMeta {
    ...
    /// Contains encrypted known plaintext "krabbykrus-vault-v1"
...
const VERIFICATION_PLAINTEXT: &[u8] = b"krabbykrus-vault-v1";
